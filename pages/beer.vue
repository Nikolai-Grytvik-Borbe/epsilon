...
let { refresh: refreshWallet, data: wallet } = useLazyFetch(
	() => `/api/users/${userId.value || -1}/wallet`,
	{
		immediate: false,
	}
)
...
async function deposit() {
	const response = await useFetch(
		() => `/api/users/${userId.value || -1}/wallet/deposit`,
		{
			method: 'POST',
			immediate: true,
			params: {
...
async function spend() {
	const response = await useFetch(
		() => `/api/users/${userId.value || -1}/wallet/spend`,
		{
			method: 'POST',
			immediate: true,
			params: {
